...
The initial source commit is `a1e09e1`; `ce78de0` adds the raw session-leader correction and both catalog filename fixtures. `c608464` commits the integrated corrections and source options. These are not a v1.0.0 release. The September 11 01:28 UTC executable passes build typing, strict policy and all four compiled gameplay/recovery cases; equal copies are in `dist/quake3-ts` and `/home/buzzkill/Projects/qfiles/q3a/quake3-ts`. It predates only the final obstacle diagnostic and later survey fixes. Raw CPU/GL product-menu switching passes after played matches in both games, including return to Quake 3. Native clipboard copy/free and paste pass on a private Xvfb display. Current release evidence is `.artifacts/releases/2026-09-11-0128/`.
